...
def _register_extra_routes(router, read_guard, write_guard) -> None:
    """Product-only actions (CSV export/import). Registered by
    build_crud_router *before* its generic "/{item_id}" route -- see that
    function's docstring for why "/export" specifically has to go here
    and can't just be appended to the router after the fact."""
