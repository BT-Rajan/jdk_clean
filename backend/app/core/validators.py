...
def not_in_past(value: date | None) -> date | None:
    """Rejects a date field that's before today (Kuwait's calendar date,
    not the server's own timezone -- see core/timezone.py). Shared by
    every schema whose date fields shouldn't be backdated --
...
def not_in_future(value: date | None) -> date | None:
    """Rejects a date field that's after today (Kuwait's calendar date)
