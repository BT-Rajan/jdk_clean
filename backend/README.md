...
There's no migration framework (Alembic, etc.) here -- `schema.sql` is
the source of truth for a fresh install, and schema changes made after
a table's initial release ship as one-off files in `migrations/`. Every
one is idempotent (guarded via `information_schema` checks), so it's
always safe to re-run.

Apply everything you're missing in one go:

```bash
python scripts/run_migrations.py
```

Or apply a single file by hand if you need to:
...
`test_data.sql` is a complete, internally-consistent dataset covering
every critical workflow the app supports -- a full order lifecycle
(feasibility through delivery), a cancelled deal, a raw-material
shortfall requiring Sales override, a product with no BOM configured,
leftover finished-goods stock recognized by a later feasibility check,
a quotation and a purchase order each awaiting admin approval, a real
MRP shortage auto-drafting a purchase order, and overdue orders/POs/
feasibility checks flagged for admin review. It was generated by
running every scenario through the real service layer (not hand-typed
SQL), so every total, discount, and tax figure is exactly what the app
itself computed.
...
Assumes `schema.sql` and all migrations are already applied. Every
table it touches is truncated before inserting, so it's safe to re-run
even against a database that already has data in those specific
tables -- it gets replaced, not duplicated or collided with. Log in as
any of the 5 seeded users (`demo_admin`, `demo_manager`, `demo_sales`,
`demo_procurement`, `demo_warehouse`), password `Demo@12345` for all
of them -- documented again at the top of the file itself.
