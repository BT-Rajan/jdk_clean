...
// containing refinements" at import time -- the previous version of
// this file called `.omit()` on the refined schema and crashed the
// entire app on load, not just this form). Both the create and edit
// schemas are built from this same base object instead, each with the
// same two cross-field checks applied independently.
...
function withStockThresholdChecks<T extends z.ZodType<{ maximum_stock: number; safety_stock: number; reorder_point: number }>>(
  schema: T,
) {
  return schema
...
export const rawMaterialSchema = withStockThresholdChecks(rawMaterialBaseSchema)
...
export const rawMaterialEditSchema = withStockThresholdChecks(rawMaterialBaseSchema.omit({ code: true }))
