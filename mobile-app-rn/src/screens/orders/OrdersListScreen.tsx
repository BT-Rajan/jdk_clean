...
import { listOrders, deleteOrder, Order, OrderStatus } from '../../api/orders';
...
export function OrdersListScreen({ navigation }: Props) {
  ...
  const load = useCallback(async (searchTerm?: string) => {
    ...
    setError(null);
    try {
      const res = await listOrders({ search: searchTerm ?? undefined, page_size: 100 });
      setOrders(res.items);
    } catch (err: any) {
      setError(err?.message ?? t('ordersList', 'loadError'));
    } finally {
      setLoading(false);
    }
  }, []);
              ...
              <GlassCard style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.rowMeta}>
                    {item.customer_name ?? '—'} · {item.order_date}
                  </Text>
                </View>
